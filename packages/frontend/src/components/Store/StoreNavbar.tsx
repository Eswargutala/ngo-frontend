    ...
    <nav className="sticky top-0 z-50 bg-[#FAF9F7]/95 dark:bg-[#1F1F1F]/95 backdrop-blur-sm border-b border-[#E5E5E5] dark:border-[#3A3A3A]">
      <div className="max-w-[1400px] mx-auto px-4 md:px-6">
        <div className="flex items-center justify-between h-16">
          {/* Logo */}
          <a href={isAuthenticated ? '/home-logged-in' : '/store'} className="flex items-center gap-2 group">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            {isAuthenticated ? (
              ...
              <div className="hidden md:flex items-center gap-2">
                <a 
                  href="/login" 
                  className="px-4 py-2 bg-transparent text-[#7A4A2E] dark:text-[#4F7C6A] rounded-lg text-[13px] font-bold hover:bg-[#F0EDE9] dark:hover:bg-[#2A2A2A] transition-all font-['Inter',sans-serif]"
                >
                  Login
                </a>
                <a 
                  href="/signup" 
                  className="px-4 py-2 bg-[#7A4A2E] dark:bg-[#4F7C6A] text-white rounded-lg text-[13px] font-semibold hover:shadow-lg transition-all font-['Inter',sans-serif]"
                >
                  Register
                </a>
              </div>
              ...
              {isAuthenticated ? (
                ...
                <div className="flex gap-2 mt-2">
                  <a
                    href="/login"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="flex-1 px-4 py-2.5 bg-transparent border border-[#7A4A2E] dark:border-[#4F7C6A] text-[#7A4A2E] dark:text-[#4F7C6A] rounded-lg text-[14px] font-semibold text-center font-['Inter',sans-serif]"
                  >
                    Login
                  </a>
                  <a
                    href="/signup"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="flex-1 px-4 py-2.5 bg-[#7A4A2E] dark:bg-[#4F7C6A] text-white rounded-lg text-[14px] font-semibold text-center font-['Inter',sans-serif]"
                  >
                    Register 
                  </a> 
                </div>
