...
export function DonationCard() {
  ...
  const t = (key: string) => donateTranslations[key]?.[language] || key;
  ...
  return (
    <div className="w-full max-w-[440px] mx-auto bg-white dark:bg-[#0F2E2A] rounded-2xl shadow-lg p-6">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="border-t border-[#E5E7EB] dark:border-[#1F4D47] pt-3 space-y-2">
          ...
          <div className="flex flex-col items-center justify-center mb-6">
             <span className="text-[13px] font-semibold text-[#1F2937] dark:text-[#F9FAFB] mb-3">Scan to Pay securely via UPI</span>
             <div className="p-2 border-2 border-[#1E7A6E]/20 dark:border-[#4FD1C5]/20 rounded-xl bg-white shadow-sm mb-2">
               <img 
             ...
             <p className="text-[11px] text-[#6B7280] dark:text-[#9CA3AF] text-center max-w-[250px]">
               After scanning and completing your payment, please submit this form to receive your certificate.
             </p>
          </div>
      ...
      {showCertificate && donationData && (
        <DonationCertificate
          isOpen={showCertificate}
          donationData={{
            name: donationData.formData.fullName,
            amount: donationData.amount,
            transactionId: `TXN${Date.now()}`,
            paymentMethod: 'UPI/Razorpay',
            email: donationData.formData.email,
            date: new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' }),
            certificateId: donationData.donationId
          }}
