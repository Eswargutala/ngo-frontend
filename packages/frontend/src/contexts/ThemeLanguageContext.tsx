...
export function ThemeLanguageProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const savedTheme = localStorage.getItem('lakhilraj-theme') as Theme | null;
    const savedLanguage = localStorage.getItem('lakhilraj-language') as Language | null;
    
    // Detect browser language
    const browserLanguage = detectBrowserLanguage();

    // Apply theme (priority: saved > default to light)
  ...
  const toggleTheme = () => {
    ...
    setTheme(newTheme);
    localStorage.setItem('lakhilraj-theme', newTheme);
  ...
  const setLanguage = (lang: Language) => {
    ...
    localStorage.setItem('lakhilraj-language', lang);
  };
